...
cap = cv2.VideoCapture('Prueba 7.MOV')
...
out = cv2.VideoWriter('output_video_7.avi', fourcc, fps, (1440, 300))
...
while cap.isOpened():
    ...
    if success:
        ...
        results = modelo.track(frame, persist=True, tracker="bytetrack.yaml")

        cv2.putText(frame, str(framecount), (50, 50), cv2.FONT_HERSHEY_SIMPLEX,1,(255, 0, 0))
        try:
            ...
            for i, (box, track_id) in enumerate(zip(boxes, track_ids)):
                ...
                if x-w/2<0:
                    ...
                    if previous_frames[track_id] is not None:
                        ...
                        prediction = comparar_imagenes(previous_frame, current_frame)
                        cv2.putText(frame, str(prediction), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0))
                    ...
                    track.append((float(x), float(y), float(h), float(w), prediction))

                    # Si la longitud de la pista es mayor que 300, retiene 90 pistas para 90 fotogramas
                    if len(track) > 400:
                        track.pop(0)

                    # Dibuja las líneas de seguimiento
                    for i in range(1, len(track)):
                        x, y, h, w, prediction = track[i]
                        x_prev, y_prev, h_prev, w_prev, prediction_prev = track[i - 1]
